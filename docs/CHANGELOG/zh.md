...
- 功能：打卡状态可切换回未记录，并支持确认删除单条或多条记录（#657）
- 功能：改进习惯详情操作，并在返回时刷新保留的详情页（#662）
- 功能：改进设置、弹窗、分组编辑和导入进度体验（#663）
- 功能：同步分组和记录时保留未知的 WebDAV 字段（#665）
- 修复：改进加载失败后的恢复流程（#662）
- 修复：确保导入结果和分组操作目标准确（#663）
- 更新意大利语翻译，感谢 Simone De Carli 在 Weblate 上的贡献（#664）

## 1.27.7+196-pre

- 功能：改进不同窗口尺寸下的弹窗和页面布局（#661）
- 修复：恢复批量打卡操作，并保留设置页滚动位置（#661）
- 修复：避免分组延迟保存完成后误关闭其他页面（#661）

## 1.27.6+195

- 功能：为宽布局新增可调节宽度的侧边导航（#653）
- 功能：改进不同窗口尺寸下的应用栏、习惯操作和批量状态修改（#655）
- 功能：在应用栏和宽布局导航中新增快捷主题模式控件（#659）
- 功能：完成导航、设置、页面标题栏、搜索、选择和习惯操作的自适应界面迁移
- 修复：让弹窗保持在应用导航之上，并在重排或重新分组习惯后正确刷新选中状态（#656）

## 1.27.3+192

- 功能：扩展自适应导航、搜索、筛选和选择操作（#646、#649）
- 功能：避免窗口控件遮挡自适应应用栏和导航区域（#647）
- 修复：改进导航安全边距、文件导入与“今日”刷新（#650）
- 修复：提升滚动时紧凑导航的稳定性（#652）
- 修复：查看习惯详情后保留习惯列表位置
- 修复：优化悬浮导航间距、深色表面、操作按钮对齐和滚动过渡
- 新增荷兰语支持并更新捷克语翻译（#651）

## 1.26.14+187

- 更新捷克语、德语、意大利语和俄语翻译及贡献者信息（#645）
- 升级至 Flutter 3.44.9，并更新依赖与构建工具（#644）
- 为搜索筛选和分组操作启用菜单动画，并为调试构建增加保护（#644）
- 改进捐赠流程、错误日志和习惯记录查询的可靠性（#644）
- 修复弹窗与嵌套详情页面连续返回异常

## 1.26.7+180

- 改进排序后端可靠性（#635）
- 更新希伯来语翻译，感谢 Omer I.S. 在 Weblate 上的贡献
- 更新意大利语翻译，感谢 nickfrn 在 Weblate 上的贡献
- 更新俄语翻译，感谢 sanda1iki 在 Weblate 上的贡献

## 1.26.5+178

- 功能：习惯计数排序、拖拽手动排序和分组管理交互升级（#621）
- 功能：为习惯列表和今日列表实现自然语言排序（#631）
- 功能：系统语言设置与本地化支持（#632）
- 改进事件处理与同步的可靠性（#625, #626, #628）
- 更新希伯来语和乌克兰语翻译，感谢 Weblate 贡献者（#620, #622, #633）
