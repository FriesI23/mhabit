...
- 修复：轻微滚动时保持紧凑导航稳定，并在明确滑动时最小化或恢复导航（#652）
- 修复：恢复矩形屏幕手机的标准悬浮导航边距，同时保留圆角屏幕间距

## 1.27.0+189-pre

- 功能：新增自适应导航栏与导航轨道、悬浮紧凑导航及习惯选择操作（#649）
- 修复：改进导航安全边距、文件导入与“今日”刷新行为（#650）
- 修复：从习惯详情返回后保留习惯列表的滚动位置
- 新增荷兰语支持（#651）
- 更新捷克语翻译，感谢 Pavel Borecki 在 Weblate 上的贡献（#651）

## 1.26.15+188-pre

- 功能：扩展自适应导航、搜索、筛选和选择操作（#646）
- 功能：避免窗口控件遮挡自适应应用栏和导航栏（#647）
