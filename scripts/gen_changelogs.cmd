...
"%POETRY_PYTHON%" bin\gen_fastlane_changelog.py "%REPO_ROOT%\CHANGELOG.md" ^
  -o "%REPO_ROOT%\fastlane\metadata\android\en-US\changelogs" --validate
if errorlevel 1 goto fail

echo Generating fastlane changelog: zh-CN
"%POETRY_PYTHON%" bin\gen_fastlane_changelog.py "%REPO_ROOT%\docs\CHANGELOG\zh.md" ^
  -o "%REPO_ROOT%\fastlane\metadata\android\zh-CN\changelogs" --validate
if errorlevel 1 goto fail

echo Generating f_store's fastlane changelog: en-US
"%POETRY_PYTHON%" bin\gen_fastlane_changelog.py "%REPO_ROOT%\CHANGELOG.md" ^
  -o "%REPO_ROOT%\android\app\src\f_store\fastlane\metadata\android\en-US\changelogs" ^
  --min-version-code 125 --with-pre --validate
if errorlevel 1 goto fail

echo Generating f_store's fastlane changelog: zh-CN
"%POETRY_PYTHON%" bin\gen_fastlane_changelog.py "%REPO_ROOT%\docs\CHANGELOG\zh.md" ^
  -o "%REPO_ROOT%\android\app\src\f_store\fastlane\metadata\android\zh-CN\changelogs" ^
  --min-version-code 125 --with-pre --validate
if errorlevel 1 goto fail
