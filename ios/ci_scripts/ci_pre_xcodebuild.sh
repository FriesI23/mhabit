...
if [[ -n "$CI_XCODEBUILD_SCHEME" ]]; then
    ...
    echo "Running: flutter build ipa --release --flavor $CI_XCODEBUILD_SCHEME"
    flutter build ipa --release --flavor "$CI_XCODEBUILD_SCHEME"
else
    echo "No scheme detected. Running: flutter build ipa --release"
